...
{
    HAL_UART_IRQHandler(&m_huart3_handle);
}
...
void HAL_UART_RxCpltCallback(UART_HandleTypeDef *huart)
{
    ...
    {
        /* 接收完成后在开启写一次中断接收 */
        HAL_UART_Receive_IT(&m_huart3_handle, (uint8_t *)&m_rx3_data, sizeof(m_rx3_data));

        TIMER_STOP(m_rx3_timer);

        if(m_is_rx3_done == true)                                  //前一帧数据还没读走，清走
        {
            m_is_rx3_done = false;
            app_fifo_flush(&m_usart3_rx_fifo);
        }

        app_fifo_puts(&m_usart3_rx_fifo, &m_rx3_data, 1);         // 接收到的数据入列

        TIMER_START(m_rx3_timer, USART1_RX_TIMEOUT);
    }
}
...
void HAL_UART_MspInit(UART_HandleTypeDef* huart)
{
  ...
  else if(huart->Instance == USART3)
  {
    ...
    gpio_init_struct.Pin        = USART3_TX_PIN | USART3_RX_PIN;
    gpio_init_struct.Mode       = GPIO_MODE_AF_PP;
    gpio_init_struct.Pull       = GPIO_NOPULL;
    gpio_init_struct.Speed      = GPIO_SPEED_FREQ_LOW;
    gpio_init_struct.Alternate  = USART3_TX_AF;
  
    HAL_GPIO_Init(USART3_TX_PORT, &gpio_init_struct);
 ...
 * Uart3 
 */
static void usart3_timer_cb(void)
{
    m_is_rx3_done = true;
}
...
int usart3_init(void)
{
    ...
    m_huart3_handle.Instance        = USART3;               /* USART3 */
    m_huart3_handle.Init.BaudRate   = USART3_BAUDRATE;      /* 波特率 */
    m_huart3_handle.Init.WordLength = UART_WORDLENGTH_8B;   /* 字长为8位数据格式 */
    m_huart3_handle.Init.StopBits   = UART_STOPBITS_1;      /* 一个停止位 */
    m_huart3_handle.Init.Parity     = UART_PARITY_NONE;     /* 无奇偶校验位 */
    m_huart3_handle.Init.HwFlowCtl  = UART_HWCONTROL_NONE;  /* 无硬件流控 */
    m_huart3_handle.Init.Mode       = UART_MODE_TX_RX;      /* 收发模式 */

    m_huart3_handle.Init.OverSampling   = UART_OVERSAMPLING_16;
    m_huart3_handle.Init.OneBitSampling = UART_ONE_BIT_SAMPLE_DISABLE;
    m_huart3_handle.Init.ClockPrescaler = UART_PRESCALER_DIV1;
    m_huart3_handle.AdvancedInit.AdvFeatureInit = UART_ADVFEATURE_NO_INIT;
    HAL_UART_Init(&m_huart3_handle);                        /* HAL_UART_Init()会使能 UART3 */

    if (HAL_UARTEx_SetTxFifoThreshold(&m_huart3_handle, UART_TXFIFO_THRESHOLD_1_8) != HAL_OK)
    {
        Error_Handler();
    }

    if (HAL_UARTEx_DisableFifoMode(&m_huart3_handle) != HAL_OK)
    {
        Error_Handler();
    }
    ...
    err_code = app_fifo_init(&m_usart3_rx_fifo, m_usart3_rx_msg, sizeof(m_usart3_rx_msg));
    if(err_code != ENONE)
    {
        return err_code;
    }
    ...
    //定义一个接收超时定时器
    TIMER_CREATE(&m_rx3_timer, true, true, usart3_timer_cb);
...
int usart3_rx(uint8_t *p_rx_data)
{
    ...
    if(m_is_rx3_done == true)
    {
        m_is_rx3_done = false;

        if(p_rx_data)
        {
            len = fifo_length(&m_usart3_rx_fifo);
            app_fifo_gets(&m_usart3_rx_fifo, p_rx_data, len);
        }
    }
